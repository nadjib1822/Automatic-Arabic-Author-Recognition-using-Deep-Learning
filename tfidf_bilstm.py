# -*- coding: utf-8 -*-
"""
Created on Tue Apr  7 15:14:30 2026
...
# ==========================================
# 2. PRÉTRAITEMENT (TF-IDF & SEQUENCES)
# ==========================================
# TF-IDF
...
# LSTM Sequences
max_words = 10000             #mots les plus fréquents du corpus
max_len = 700                 #taille du vocabulaire utilisable par le modèle
...
emb = Embedding(input_dim=max_words, output_dim=128)(input_lstm)
...
tfidf_dense = Dense(256, activation='relu')(input_tfidf)
...
history = model.fit([X_train_seq, X_train_tfidf], y_train, 
                    ...
                    epochs=40, batch_size=8, verbose=1)
...
# ==========================================
# 6. EXPORT DES SORTIES PAR COUCHE (CSV)
# ==========================================

# 1. Création du modèle extracteur
extractor = Model(inputs=model.input, outputs=[lstm_out, tfidf_out, output])

# 2. Extraction des données sur le jeu de test
print("\nExtraction des vecteurs pour chaque couche...")
l_val, t_val, o_val = extractor.predict([X_test_seq, X_test_tfidf])

# --- FICHIER 1 : LSTM OUT ---
df_lstm = pd.DataFrame(l_val, columns=[f'LSTM_Dim_{i+1}' for i in range(l_val.shape[1])])
df_lstm.insert(0, 'Auteur_Reel', [sorted_authors[i] for i in np.argmax(y_test, axis=1)])
df_lstm.to_csv("sortie_lstm_out.csv", index=False, encoding='utf-8-sig')

# --- FICHIER 2 : TF-IDF OUT ---
df_tfidf = pd.DataFrame(t_val, columns=[f'TFIDF_Dim_{i+1}' for i in range(t_val.shape[1])])
df_tfidf.insert(0, 'Auteur_Reel', [sorted_authors[i] for i in np.argmax(y_test, axis=1)])
df_tfidf.to_csv("sortie_tfidf_out.csv", index=False, encoding='utf-8-sig')

# --- FICHIER 3 : OUTPUT ---
df_output = pd.DataFrame(o_val, columns=[f'Prob_{name}' for name in sorted_authors])
df_output.insert(0, 'Auteur_Reel', [sorted_authors[i] for i in np.argmax(y_test, axis=1)])
df_output['Prediction'] = [sorted_authors[i] for i in np.argmax(o_val, axis=1)]
df_output.to_csv("sortie_final_output.csv", index=False, encoding='utf-8-sig')

print("\n" + "="*50)
print("✅ TROIS FICHIERS CSV GÉNÉRÉS :")
print("1. sortie_lstm_out.csv    (Signature séquentielle)")
print("2. sortie_tfidf_out.csv   (Signature lexicale)")
print("3. sortie_final_output.csv (Probabilités finales)")
print("="*50)
